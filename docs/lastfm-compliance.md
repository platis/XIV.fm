...
- The plugin attributes listening data to Last.fm. `/xivfm lastfm` opens the current Last.fm track page, or the linked user profile when no track URL is cached. Holding Alt and clicking a listening card opens only server-supplied HTTPS links whose normalized host is `last.fm` or one of its subdomains.
