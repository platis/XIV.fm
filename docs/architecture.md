...
Cards use the game's pose-aware nameplate world position plus a 0.7-yalm world-space safety height and project it through Dalamud, so sitting, lying, mounted, and other animated states do not inherit a fixed standing-height offset or a camera-dependent pixel gap. The narrowly scoped, read-only FFXIVClientStructs boundary and its safety constraints are recorded in [`ADR 0001`](adr/0001-pose-aware-nameplate-anchor.md). The anchor adapter must be tested against actual nameplates across races, poses, UI scales, camera angles, world travel, crowded locations, and hidden nameplate conditions.
