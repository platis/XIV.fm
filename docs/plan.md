...
- [ ] Finalize card typography, sizing, animation, accessibility, scale, and obstruction behavior; add live album art only if the provider grants permission.
  - Product-owner feedback promoted the compact placeholder into the final-design baseline: 222×58 px, 42×42 conditional artwork with no fallback placeholder, a bold title, subdued artist attribution, no red plugin heading, and a 0.4-yalm world-space safety height. The SVG records this current layout.
