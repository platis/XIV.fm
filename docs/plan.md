...
- [ ] Finalize card typography, sizing, animation, accessibility, scale, and obstruction behavior; add live album art only if the provider grants permission.
  - Product-owner feedback promoted the compact placeholder into the final-design baseline: 244×64 px, 46×46 conditional artwork with no fallback placeholder, a bold title, subdued artist line, ellipsized overflow, no plugin/provider label, and a 0.4-yalm world-space safety height. The SVG records this current layout.
