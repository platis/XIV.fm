...
- [ ] Finalize card typography, sizing, animation, accessibility, scale, and obstruction behavior; add live album art only if the provider grants permission.
  - Product-owner feedback promoted the compact placeholder into the final-design baseline: 293×64 px, 46×46 conditional artwork with no fallback placeholder, a bold title, larger subdued artist line, bottom-aligned text, ellipsized overflow, no plugin/provider label, and a 0.4-yalm world-space safety height. The SVG records this current layout.
